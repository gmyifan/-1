...
export class DocumentService {
  ...
  private formatCorrectAnswer(question: Question): string {
    ...
    if (question.type === 'trueFalse') {
      const idx = Array.isArray(question.options) ? question.options.indexOf(question.correctAnswer) : -1;
      const letter = idx >= 0 ? String.fromCharCode(65 + idx) : '';
      return letter ? `${letter} ${question.correctAnswer}` : question.correctAnswer;
    }
    const letters = (question.correctAnswer.match(/[A-H]/gi) || []).map(c => c.toUpperCase());
    if (letters.length && Array.isArray(question.options) && question.options.length) {
  ...
  private generateQuestionTables(questions: Question[]): Paragraph[] {
    ...
    questions.forEach((question, index) => {
      ...
      paragraphs.push(
        new Paragraph({
          text: `正确答案: ${this.formatCorrectAnswer(question)}`,
          spacing: { after: 100 }
        })
      );

...
export const StartScreen: React.FC<StartScreenProps> = ({ onViewWrongQuestions }) => {
  const { startExam } = useExam();
  // 难度：basic=基础达标级，applied=应用实践级
  const [difficulty, setDifficulty] = React.useState<'basic' | 'applied'>('applied');

  const getCounts = () => {
    switch (difficulty) {
      case 'basic':    return { singleCount: 50, trueFalseCount: 20, multipleCount: 20 };
      case 'applied':  return { singleCount: 50, trueFalseCount: 20, multipleCount: 20 };
    }
  };
  ...
  const handleStartExam = async () => {
    try {
      ...
      const examPaper = await questionBankService.generateExamPaper({ ...counts, difficulty });

      if (!examPaper.questions || examPaper.questions.length === 0) {
        alert('题库解析失败：未读取到题目。请检查题库格式或稍后重试。');
        return;
      }
      
      await startExam(examPaper);
    } catch (error) {
        ...
        <div className="start-screen__content">
          ...
          <div className="start-screen__actions">
            <div className="start-screen__difficulty">
              <label className="start-screen__difficulty-label">难度选择：</label>
              <label><input type="radio" name="diff" checked={difficulty==='basic'} onChange={() => setDifficulty('basic')} /> 基础达标级</label>
              <label><input type="radio" name="diff" checked={difficulty==='applied'} onChange={() => setDifficulty('applied')} /> 应用实践级</label>
            </div>
            ...
            {onViewWrongQuestions && (
              <Button
                ...
                className="start-screen__wrong-questions-button"
              >
                查看错题集
              </Button>
            )}
